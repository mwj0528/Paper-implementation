...
def load_generators_for_inference():
    ...
    checkpoint = torch.load('/mnt/MW/checkpoints/CycleGAN/checkpoint_latest.pth')
...
def tensor_to_arr(tensor):
    array = tensor.squeeze().cpu().numpy().transpose(1, 2, 0)
...
# 디렉토리 설정
pdw_dir = "/mnt/MW/data/IXI datasets/png/test/PDw"
t2w_dir = "/mnt/MW/data/IXI datasets/png/test/T2w"
output_dir = "/mnt/MW/CycleGAN_MR/test_results"
...
for filename in common_files:
    ...
    # 이미지 불러오기
    sample_image = Image.open(pdw_path).convert('RGB')
    gt_image = Image.open(t2w_path).convert('RGB')
    
    # 전처리 후 PyTorch 텐서로 변환
    ...
    # 2x2 시각화 후 저장
    output_path = os.path.join(output_dir, f"PDw_to_T2w_{filename}")
    ...
    plt.subplot(2,2,3)
    plt.imshow(recovered_PDw_pil)
    plt.title("Recovered PDw")
    plt.axis('off')

    plt.subplot(2,2,4)
    plt.imshow(fake_T2w_pil)
    plt.title("Fake T2w")
    plt.axis('off')
...
for filename in common_files:
    ...
    # 이미지 불러오기
    sample_image = Image.open(t2w_path).convert('RGB')
    gt_image = Image.open(pdw_path).convert('RGB')
    ...
    # 2x2 시각화 후 저장
    output_path = os.path.join(output_dir, f"T2w_to_PD2w_{filename}")
    plt.figure(figsize=(8, 8))
    ...
    plt.subplot(2,2,3)
    plt.imshow(recovered_T2w_pil)
    plt.title("Recovered T2w")
    plt.axis('off')

    plt.subplot(2,2,4)
    plt.imshow(fake_PDw_pil)
    plt.title("Fake PDw")
    plt.axis('off')
