...
def load_generators_for_inference():
    ...
    checkpoint = torch.load('/mnt/MW/checkpoints/pix2pix_MR/checkpoint_latest.pth')
...
def tensor_to_arr(tensor):
    ...
    array = tensor.squeeze().cpu().numpy().astype(np.float32)
