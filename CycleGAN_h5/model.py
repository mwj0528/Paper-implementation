...
class MRDataset(Dataset):
    def __init__(self, pdw_dir, t2w_dir, transform=None):
        ...
        self.pdw_files = [os.path.join(pdw_dir, f) for f in os.listdir(pdw_dir) if f.endswith('.hdf5')]  # .hdf5 파일만 선택
        self.t2w_files = [os.path.join(t2w_dir, f) for f in os.listdir(t2w_dir) if f.endswith('.hdf5')]  # .hdf5 파일만 선택
    ...
    def __getitem__(self, index):
        ...
        t2w_img = Image.fromarray(t2w_img, mode='L') # 이미지로 변환s
...
def save_checkpoint(epoch, G_PDw2T2w, G_T2w2PDw, D_PDw, D_T2w,
    ...
    save_path = os.path.join('/mnt/MW/checkpoints/CycleGAN_h5/', f'checkpoint_latest.pth')
...
def train():
    ...
    # 체크포인트가 있다면 로드
    if os.path.exists('/mnt/MW/checkpoints/CycleGAN_h5/checkpoint_latest.pth'):
        start_epoch = load_checkpoint('/mnt/MW/checkpoints/CycleGAN_h5/checkpoint_latest.pth',
